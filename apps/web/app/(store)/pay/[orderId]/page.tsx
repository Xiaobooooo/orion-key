...
export default function PaymentPage({params}: { params: Promise<{ orderId: string }> }) {
    ...
    // USDT 支付判断 & 参数
    const isUsdtPayment = paymentMethod.startsWith("usdt_") || paymentMethod.startsWith("bnb_") || paymentMethod.startsWith("trx_")
    ...
    const chainDisplayName = usdtChain.includes("bsc") ? "BSC" :
        usdtChain.includes("tron") ? "TRON" :
            usdtChain.includes("bep20") ? "BEP-20" :
                usdtChain.includes("trc20") ? "TRC-20" : usdtChain
            ...
            {timeLeft < 0 ? "--:--" : formatTime(timeLeft)}
                ...
                {isUsdtPayment ? (
                    ...
                    <div className="flex w-full flex-col items-center gap-3">
                        {/* ① 品牌标签 — 网络类型 */}
                        <div
                            className="my-0.5 inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-4 py-1.5 transition-all duration-200">
                        ...
                        {/* ②③ 转账金额 + 收款地址 — 合并卡片 */}
                        <div
                            className="flex w-full max-w-sm flex-col rounded-xl border border-border/60 bg-card shadow-sm transition-all duration-200 hover:border-border hover:shadow-md">
                            {/* 转账金额 */}
                            <div className="flex flex-col gap-1 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-2">
                                <span
                                    className="text-sm text-muted-foreground">{t("payment.usdt.amount")}（{t("payment.usdt.amountHint")}）</span>
                    ...
                    <Copy className="h-4 w-4"/>
                        ...
                        >
                            {walletAddress ? (
                                ...
                                <div
                                    className="flex h-[140px] w-[140px] flex-col items-center justify-center gap-2 text-muted-foreground sm:h-[160px] sm:w-[160px]">
